...
- 勾选“替换现有图像”时，在 Emby 刷新前通知 ETK 按当前图片语言优先级重新选择并持久化 TMDb 图片。
- Emby“搜索图像”每次实时查询 TMDb 全量候选，返回语言、分辨率、评分和票数，不受数据库单张图片缓存限制。
- 图片和人物头像统一通过 ETK 图片代理下载，避免 Emby 无法直连 TMDb 时刷新丢图。
...
- 按 Emby 媒体库的合集导入和最小尺寸设置创建 BoxSet，创建成功后主动通知 ETK 激活对应合集缓存。
- 直接监听 Emby 播放、用户数据、权限、合集和手动元数据/图片编辑事件并上报 ETK，无需配置 Emby Webhook。
- 按 ETK 槽位和洗版优先级格式化 Emby 多版本下拉展示名，并保持槽位顺序。
...
8. 插件只把 `MetadataEdit` 和带手动图片 API 标记的 `ImageUpdate` 上报 ETK；刷新导入和插件自身回填会被过滤。
...
首次安装仍需手动放入 DLL。之后可在 Emby 的“计划任务”中运行“更新 ETK MediaInfo Bridge”，
